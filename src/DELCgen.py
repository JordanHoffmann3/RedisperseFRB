...
class Lightcurve(object):
    ...
    def Periodogram(self):
        ...
        freq = np.arange(1, int(self.length/2) + 1).astype(float)/\
                                                         (self.length*self.tbin)
        shortPeriodogram = np.take(periodogram,np.arange(1,int(self.length/2) +1))
    ...
    def Plot_PDF(self,bins=None,norm=True,force_scipy=False):
        ...
        plt.hist(self.flux,bins=self.bins,Density=norm)
        #kappa,theta,lnmu,lnsig,weight
        if self.pdfFit:
    ...
    def Plot_Stats(self,bins=None,norm=True,force_scipy=False):
        ...
        plt.subplot(3,1,2)
        if bins:
            plt.hist(self.flux,bins=bins,density=norm)        
        else:
            if self.bins == None:
                self.bins = OptBins(self.flux)
            plt.hist(self.flux,bins=self.bins,density=norm)        
...
def Comparison_Plots(lightcurves,bins=None,norm=True, names=None,
    ...
    for lc in lightcurves:
        ...
        # lightcurve
        ax_lc = fig.add_subplot(3,n,1+i)
        ax_lc.scatter(lc.time,lc.flux,color=colors[c], s=10,alpha=0.5)
        ax_lc.ticklabel_format(style='sci', axis='x', scilimits=(0,0))
        ax_lc.set_title(names[i])
        
        ax_lc.minorticks_on()
        ax_lc.tick_params(axis='both', which='major', labelsize=12, length=9)
        ax_lc.tick_params(axis='both', which='minor', length=4.5)
        ax_lc.tick_params(axis='both', which='both', direction='in',right=True,top=True)

        # PDF
        ax_pdf = fig.add_subplot(3,n,n+1+i)
        ...
        if bins == None:
            if lc.bins == None:
                lc.bins = OptBins(lc.flux)
            bins = lc.bins
        ax_pdf.hist(lc.flux,bins=bins,density=norm,color=colors[c],alpha=0.5,edgecolor='w')
        if lightcurves[-1].pdfFit:
            ...
            if mix:
                ax_pdf.plot(x,lightcurves[-1].pdfModel.Value(x,lightcurves[-1].pdfFit['x']),
                       'red', linewidth=3) 
            elif scipy:
                ax_pdf.plot(x,lightcurves[-1].pdfModel.pdf(x,lightcurves[-1].pdfFit['x']),
                       'red', linewidth=3) 
            else:
                ax_pdf.plot(x,lightcurves[-1].pdfModel(x,*lightcurves[-1].pdfFit['x']),
                       'red', linewidth=3 ) 
        ax_pdf.set_title("PDF")

        ax_pdf.minorticks_on()
        ax_pdf.tick_params(axis='both', which='major', labelsize=12, length=9)
        ax_pdf.tick_params(axis='both', which='minor', length=4.5)
        ax_pdf.tick_params(axis='both', which='both', direction='in',right=True,top=True)

        
        # PSD
        ax_psd = fig.add_subplot(3,n,2*n+1+i)
        ax_psd.plot(lc.periodogram[0],lc.periodogram[1],'-',lw=0.5,color=colors[c])
        ## commenting this bit out as I'm not sure how it would've worked in Py2.
        ...
        ax_psd.set_xlim([0.8*min(lc.periodogram[0]),1.2*max(lc.periodogram[0])])
        ax_psd.set_ylim([0.8*min(lc.periodogram[1]),1.2*max(lc.periodogram[1])])

        ax_psd.minorticks_on()
        ax_psd.tick_params(axis='both', which='major', labelsize=12, length=9)
        ax_psd.tick_params(axis='both', which='minor', length=4.5)
        ax_psd.tick_params(axis='both', which='both', direction='in',right=True,top=True)
